...
while not touchSensorPressed:
    ...
    # Predict
    x_inter = F * x_pre + B * u
    P_inter = P_pre + Q  # SM: can go
    # Update
    S = P_inter + R  # SM: can go
    K = P_inter / S  # SM: can go
    y = gyroRate
    x_curr = x_inter + K * (y - x_inter)
    P_curr = (1 - K) * P_inter  # SM: can go
    # shift
    x_pre = x_curr
    P_pre = P_curr  # SM: can go
    gyroRate = x_curr
    ...
    #Log Plot
    measure = np.append(measure, gyroRate)
...
# Save File
sio.savemat('measure.mat', {'vect': measure})
