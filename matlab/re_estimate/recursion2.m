function [F,H,B,Q,R,pi1,V1,loglike,num_iter,xhat1i] = recursion2(measure,input,F,H,B,Q,R,pi1,V1,num_iter,xhat1i)
% This function verifies whether EM algorithm finally converges to "true
% value"
% Formulas derived by A.W.Blocker
...
for t = 1:T
    if t == 1
        ...
        xtt1(t) = F*xtt(t-1) + B*u(t); %starts from x_2^1
...
F = sum(Ptt1(2:T)-B*u(2:T).*xhat(1:T-1))/sum(Pt(1:T-1));
%F = 1;
