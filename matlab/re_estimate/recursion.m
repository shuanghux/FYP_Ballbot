function [F,H,B,Q1,R,pi1,V1,loglike,num_iter,xhat1i] = recursion(measure,input,F,H,B,Q,R,pi1,V1,num_iter,xhat1i)
...
%Equation 4
%loglikelihood
loglike = -sum((y-H*xhat).^2)/(2*abs(R)) - T/2*log(abs(R))...
    -sum((xhat(2:T)-F*xhat(1:T-1)-B*u(1:T-1)).^2)/(2*abs(Q)) - (T-1)/2*log(abs(Q))...
...
Q = 1/(T-1)*sum(Pt(2:T) - 2.*xhat(2:T).*u(1:T-1)*B - F*Ptt1(2:T) + F*xhat(1:T-1).*u(1:T-1)*B  + B*(u(1:T-1).^2)*B);
Q1 = 1/(T-1)*sum((xhat(2:T)-F*xhat(1:T-1)-B*u(1:T-1)).^2 +Pt(2:T) - 2*F*Ptt1(2:T) + F*Pt(1:T-1)*F);
