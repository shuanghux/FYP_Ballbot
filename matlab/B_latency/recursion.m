...
% This function verifies whether EM algorithm finally converges to "true
% value"
% Formulas derived by Simon Maskell
...
%% Introduce a latancy control vector
% control signals may exists for 3 time intervals
U_vec = zeros(3,T);
U_vec(:,1)=[0;0;u(1)];
U_vec(:,2)=[0;u(1);u(2)];
for t = 3:T
...
for t = 1:T
    if t == 1
        ...
        xtt1(t) = F*xtt(t-1) + sum(B*U_vec(:,t-1)); %starts from x_2^1
...
for t = T:-1:2
    ...
    xtT(t-1) = xtt(t-1) + J(t-1)*(xtT(t)-F*xtt(t-1)-sum(B*U_vec(:,t-1)));
...
loglike = -sum((y-H*xhat).^2)/(2*R) - T/2*log(abs(R))...
    -sum((xhat(2:T)-F*xhat(1:T-1)-B*U_vec(:,1:T-1)).^2)/(2*abs(Q)) - (T-1)/2*log(abs(Q))...
...
%F = sum(Ptt1(2:T)-B*U_vec(:,1:T-1).*xhat(1:T-1))/sum(Pt(1:T-1));
F = 1;
%new version of B and Q
%B = sum((xhat(2:T)-F*xhat(1:T-1)).*u(1:T-1))/sum((u(1:T-1).^2));

%B = sum(((xhat(2:T)'-F*xhat(1:T-1)').*U_vec(:,1:T-1)'))./sum(U_vec(:,1:T-1)'.*U_vec(:,1:T-1)');
B1 = sum(((xhat(2:T)'-F*xhat(1:T-1)').*U_vec(1,1:T-1)'))./sum(U_vec(1,1:T-1)'.*U_vec(1,1:T-1)');
B2 = sum(((xhat(2:T)'-F*xhat(1:T-1)').*U_vec(2,1:T-1)'))./sum(U_vec(2,1:T-1)'.*U_vec(2,1:T-1)');
B3 = sum(((xhat(2:T)'-F*xhat(1:T-1)').*U_vec(3,1:T-1)'))./sum(U_vec(3,1:T-1)'.*U_vec(3,1:T-1)');

B = [B1 B2 B3];
%B = [-0.001 -0.001 -0.002];
%B = 0.72;
%Update state noise covariance
%Q = 1/(T-1)*sum(Pt(2:T) - 2*xhat(2:T).*(B*U_vec(:,1:T-1))  ...
%    - F*Ptt1(2:T) + F*xhat(1:T-1).*(B*U_vec(:,1:T-1))  + (B*U_vec(:,1:T-1)).^2);
Q = 0.1;
